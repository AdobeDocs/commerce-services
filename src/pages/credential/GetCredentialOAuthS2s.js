...
const GetCredentialOAuthS2s = () => {
  ...
  return (
    ...
    <GetCredential className="getCredentialContainer" templateId="6887db51a6867b6202ef2f9c" productName='Adobe Commerce Optimizer' >
      ...
      <GetCredential.Form title="Get credentials" paragraph="Create credentials that you can use to call the catalog data ingestion REST API.<br/><br/>You also need to know your sandbox tenant ID to use the API." className="formClass">
      ...
      <GetCredential.Card title="Your credential is ready to use" developerConsoleManage="Manage on Developer Console" className="card_developer_console" devConsoleDirection="/console" isCollapsable="true">
        ...
        <GetCredential.Card.CredentialDetails heading={(<>Credential details<br/><br/> <span style="font-weight:normal"><small>Copy your tenant ID from the Commerce Cloud Manager and paste it in the <code>tenantID</code> field in the <strong>Try it</strong> panel below. You only need to do this once; future requests will use it automatically. <a href="https://developer.adobe.com/commerce/services/optimizer/data-ingestion/using-the-api/">Learn more</a>.</small></span></>)} orderBy="ClientId,ImsOrgID">
