  ...
  return (
    ...
    <GetCredential className="getCredentialContainer" templateId="6887db51a6867b6202ef2f9c" productName='Adobe Commerce Optimizer' >
      ...
      <GetCredential.Card title="Your credential is ready to use" developerConsoleManage="Manage on Developer Console" className="card_developer_console" devConsoleDirection="/console" isCollapsable="true">
        ...
        <GetCredential.Card.AccessToken helpText="" buttonLabel="Generate and copy token" heading={(<>Access token<br/><br/> <span style="font-weight:normal"><small>Click the following button to generate an access token with the Client ID (<code>x-api-key</code>) and Organization ID (<code>x-gw-ims-org-id</code>) displayed in the <strong>Credential details</strong> section below.<br/><br/>After copying the access token, paste it into the <strong>Authorization</strong> header parameter of any API request in the <strong>Try it</strong> panel below. You only need to paste it once—after that, all other Authorization header fields in subsequent requests will be automatically populated for you.</small></span></>)}/>
