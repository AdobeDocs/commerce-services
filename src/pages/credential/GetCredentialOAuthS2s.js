  ...
  return (
    ...
    <GetCredential className="getCredentialContainer" templateId="6887db51a6867b6202ef2f9c" productName='Adobe Commerce Optimizer' >
      ...
      <GetCredential.Card title="Your credential is ready to use" developerConsoleManage="Manage on Developer Console" className="card_developer_console" devConsoleDirection="/console" isCollapsable="true">
        ...
        <GetCredential.Card.CredentialDetails heading={(<>Credential details<br/><br/> <span style="font-weight:normal"><small>In addition to the access token, you must manually copy and paste a tenant ID from the Commerce Cloud Manager into the <code>tenantID</code> field in the <strong>Try it</strong> panel below. See the <a href="https://developer.adobe.com/commerce/services/optimizer/data-ingestion/using-the-api/">Get started</a> documentation for help finding your tenant ID. You only need to do this once; future requests will reuse the value.<br/><br/>The client ID and Organization ID below are used to generate the access token.</small></span></>)} orderBy="ClientId,ImsOrgID">
      ...
      <GetCredential.Return title="Previously created projects" paragraph="Select a project and access your existing credentials for Adobe Commerce Optimizer." className="card_developer_console" isCollapsable="true">
        ...
        <GetCredential.Return.CredentialDetails heading={(<>Credential details<br/><br/> <span style="font-weight:normal"><small>To try the API, you also need the tenant ID for an Optimizer instance in the sandbox environment. Copy the <code>instance ID</code> from the Commerce Cloud Manager into the <code>tenantID</code> field in the <strong>Try it</strong> panel below. For help, see <a href="https://developer.adobe.com/commerce/services/optimizer/data-ingestion/using-the-api/">Get started</a>. You only need to do this once; future requests will reuse the value.<br/><br/>The  following client ID and Organization ID values are used to generate the access token:</small></span></>)} orderBy="ClientId,ImsOrgID">
